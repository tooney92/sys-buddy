...
def role_prompt(
    ...
    return (
        ...
        "(e.g. `pc #3` proposes the contract ON todo 3, signed by its parties). Every task is delivered by one or more todos, so `#N` always names one — a task with a single deliverable simply has one todo. No `locked?` — the broker pushes the lock to you; `wm` catches it\n"
